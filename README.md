...
<body oncontextmenu="return false">
    ...
    <div class="max-w-md mx-auto px-4 py-6">
        ...
        <div class="flex space-x-2 mb-6 overflow-x-auto pb-2 no-scrollbar font-bold text-sm">
            ...
            <button onclick="selectWeek('total2')" id="tab-total2" class="week-tab flex-shrink-0 px-5 py-2 bg-white rounded-full shadow-sm border border-slate-100">Week 5 누적</button>
        </div>
    ...
    <script>
        const allData = [
            /* Day 1 ~ 15 데이터는 생략하지 않고 모두 포함되어 있습니다. (가독성을 위해 주석 처리 없이 배열에 유지) */
        ...
        let selectedWeekMode = 'w5'; // 시작 시 Week 5로 설정
        let quizPool = [];
        let currentIndex = 0;
        ...
        function selectWeek(mode) {
            ...
            else if(mode === 'total2') titleEl.innerText = "WEEK 5~ (Month 2 Accumulated)";
        }
        ...
        function startQuiz(difficulty) {
            ...
            else if (selectedWeekMode === 'total2') weekFiltered = allData.filter(item => item.d >= 21 && item.d <= 28); // 추후 Day 28까지 확장 대비
            ...
            // 랜덤 10문제 추출 (데이터가 10개 미만이면 전체 출제)
